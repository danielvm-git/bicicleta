...
async function verify() {
  ...
  const mockPrice = (parseFloat(comp.price) + 10.50).toFixed(2);
  ...
  // 3. Verify results
  const [updated] = await db.select().from(components).where(eq(components.id, comp.id));
  const history = await db.select().from(componentPrices).where(eq(componentPrices.componentId, comp.id));
