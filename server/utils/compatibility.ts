...
export const validateBuild = (components: BikeComponent[]): CompatibilityIssue[] => {
  ...
  if (frame?.axleType?.includes('Boost 148mm')) {
    ...
    if (nonBoostHub) {
      issues.push({
        severity: 'error',
        message: 'Cubo traseiro incompatível: Quadro Boost exige cubo Boost 148mm.',
        category: 'Cubo'
      })
    }
  }
  ...
  if (frame?.steeringType && suspension?.steeringType) {
    if (frame.steeringType === 'Over' && suspension.steeringType === 'Tapered') {
      issues.push({
        severity: 'error',
        message: "Direção incompatível: Quadro 'Over' não suporta suspensão 'Tapered'.",
        category: 'Suspensão'
      })
    }
    if (frame.steeringType === 'Tapered' && suspension.steeringType === 'Over') {
      issues.push({
        severity: 'warning',
        message: "Aviso: Suspensão 'Over' em quadro 'Tapered' exige caixa de direção com redutor.",
        category: 'Suspensão'
      })
    }
  }
  ...
  if (headset?.model && frame?.steeringType) {
    ...
    if (frame.steeringType === 'Over' && isHeadsetTapered) {
      issues.push({
        severity: 'error',
        message: "Caixa de Direção incompatível: Quadro 'Over' (44mm) não suporta caixa 'Tapered'.",
...
export const checkComponentCompatibility = (component: BikeComponent, build: BikeComponent[]): { compatible: boolean; reason?: string } => {
  const frame = build.find(c => c.category === 'Quadro')
  
  // 1. Drivetrain speeds
  const drivetrainCategories = ['Cassete', 'Câmbio Traseiro', 'Câmbio Dianteiro', 'Alavanca de Câmbio', 'Transmissão']
  if (drivetrainCategories.includes(component.category) && component.speeds) {
    const existing = build.find(c => drivetrainCategories.includes(c.category) && c.speeds)
  ...
  if (component.category === 'Quadro' && component.axleType?.includes('Boost 148mm')) {
    const hub = build.find(c => c.category === 'Cubo')
