...
export function inferTechnicalSpecs(
  model: string,
  spec?: string,
  link?: string
): {
  ...
  if (
    combined.includes("tapered") ||
    combined.includes("cônica") ||
    combined.includes("is42/52") ||
    combined.includes("zs44/56")
  )
    result.steeringType = "Tapered";
  else if (combined.includes("mega over") || combined.includes("44mm"))
    result.steeringType = "Over";
  else if (combined.includes("over")) result.steeringType = "Over";

  // Axle: Boost, 148, 142, 135, E-thru, QR
  if (combined.includes("110x15") || combined.includes("110mm"))
    result.axleType = "Boost 110mm";
  else if (
    combined.includes("boost") ||
    combined.includes("148x12") ||
    combined.includes("148mm")
  )
    result.axleType = "Boost 148mm";
  else if (combined.includes("142x12") || combined.includes("142mm"))
    result.axleType = "142x12mm";
  else if (
    combined.includes("9mm") ||
    combined.includes("qr") ||
    combined.includes("quick release")
  )
    result.axleType = "Quick Release";

  // Suspension Travel: 100mm, 120mm, 80mm
  const travelMatch = combined.match(/(\d+mm)/);
  if (
    travelMatch &&
    (combined.includes("garfo") ||
      combined.includes("suspensão") ||
