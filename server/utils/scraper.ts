...
export async function scrapeML(url: string): Promise<ScrapeResult> {
  try {
    ...
    if (metaPrice) {
      ...
      return isNaN(price) ? { price: null, status: 'not_found' } : { price, status: 'success' };
    }

    // 2. Try specific classes as fallback
    const priceFraction = $('.ui-pdp-price__main-container .andes-money-amount__fraction').first().text();
    const priceCents = $('.ui-pdp-price__main-container .andes-money-amount__cents').first().text();
    
    if (priceFraction) {
      const price = parseFloat(priceFraction.replace(/\./g, '').replace(',', '.')) + (parseFloat(priceCents || '0') / 100);
      return isNaN(price) ? { price: null, status: 'not_found' } : { price, status: 'success' };
    }
...
export async function scrapePrice(url: string): Promise<ScrapeResult> {
  if (!url || url === '-') return { price: null, status: 'error', error: 'Invalid URL' };
