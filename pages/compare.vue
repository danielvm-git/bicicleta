...
const { data: templates, pending } = await useFetch("/api/bikes/templates");
...
const templateTotalWeight = computed(() => {
  ...
  return selectedTemplate.value.bikeComponents.reduce(
    (total: number, bc: any) => {
      const weight =
...
const groupedCustom = computed(() => {
  ...
  Object.values(bike.state.components).forEach((comp) => {
...
const groupedTemplate = computed(() => {
  ...
  selectedTemplate.value.bikeComponents.forEach((bc: any) => {
...
const allCategories = computed(() => {
  const cats = new Set([
    ...Object.keys(groupedCustom.value),
    ...Object.keys(groupedTemplate.value),
  ]);
  ...
  <UContainer class="py-8">
      ...
      >
        ...
        <tbody>
          <tr v-for="category in allCategories" :key="category">
            ...
            <td
              class="border border-gray-200 dark:border-gray-700 p-3 align-top"
            >
              <div v-if="groupedCustom[category]" class="space-y-1">
                <div v-for="comp in groupedCustom[category]" :key="comp.id">
                  ...
                  <div
                    class="flex flex-wrap gap-x-2 gap-y-0.5 text-xs text-gray-500"
                  >
                    <span
                      v-if="comp.speeds"
                    ...
                    <span v-if="comp.weight" class="italic"
                      >{{ comp.weight }}kg</span
                    >
        ...
        <tfoot>
          <tr
            class="bg-gray-50 dark:bg-gray-800 font-bold border-t-2 border-gray-200 dark:border-gray-700"
          >
            ...
            <td class="border border-gray-200 dark:border-gray-700 p-3">
              {{
                bike.totalWeight > 0
                  ? `${bike.totalWeight.toFixed(3)} kg`
                  : "Sob consulta"
              }}
            </td>
            <td class="border border-gray-200 dark:border-gray-700 p-3">
              {{
                templateTotalWeight > 0
                  ? `${templateTotalWeight.toFixed(3)} kg`
                  : "Sob consulta"
              }}
            </td>
          </tr>
          <tr class="bg-gray-50 dark:bg-gray-800 font-bold">
            <td class="border border-gray-200 dark:border-gray-700 p-3 text-lg">
              TOTAL
            </td>
            <td
              class="border border-gray-200 dark:border-gray-700 p-3 text-primary text-xl"
            >
              {{ formatCurrency(bike.totalPrice) }}
            </td>
            <td
    ...
    >
      ...
      <p class="text-gray-500">
        Selecione um template comercial para iniciar a comparação.
      </p>
    </div>
  </UContainer>
</template>
