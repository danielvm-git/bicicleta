...
import { cleanWeight, inferTechnicalSpecs, cleanPrice, normalizeCategory } from '../../server/utils/parser';
...
describe('Parser Utils', () => {
  ...
  describe('inferTechnicalSpecs', () => {
    ...
    it('should infer steering type', () => {
      expect(inferTechnicalSpecs('Quadro Absolute Wild Tapered').steeringType).toBe('Tapered');
      expect(inferTechnicalSpecs('Suspensão Absolute Prime Cônica').steeringType).toBe('Tapered');
    });

    it('should infer axle type', () => {
      expect(inferTechnicalSpecs('Cubo Boost 148x12').axleType).toBe('Boost 148mm');
      expect(inferTechnicalSpecs('Quadro com eixo passante 142mm').axleType).toBe('142x12mm');
    });

    it('should infer suspension travel', () => {
      expect(inferTechnicalSpecs('Suspensão 100mm com trava').suspensionTravel).toBe('100mm');
    });
  });
